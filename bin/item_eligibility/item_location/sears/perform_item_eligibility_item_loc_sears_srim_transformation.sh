##############################################################################################################################
#
#       Script Name   : perform_item_eligibility_item_loc_sears_srim_transformation.sh
#
#       Description   : This is a wrapper script for invoking all hadoop jobs for item_eligibility_sears_srim transformation
#
#       Author        : Mudit Mangal
...
# Transformations of SRIM DATA

pig -Dudf.import.list=$PKG_NAME -logfile $HDIDRP_LOG_FILE_DIR/pig -p batch_id=$1 -p WORK_DIR=$WORK_DIR -p SMITH_DIR=$SMITH_DIR -p GOLD_DIR=$GOLD_DIR -param_file $PIG_PARAM_PATH/item_eligibility_sears.param  -m $PIG_SCHEMA_PATH/smith__idrp_eligible_item_loc_srim_join.param  -m $PIG_SCHEMA_PATH/smith__idrp_eligible_loc.param  $PIG_SCRIPT_PATH/item_eligibility_item_loc_sears_srim_transformation.pig
