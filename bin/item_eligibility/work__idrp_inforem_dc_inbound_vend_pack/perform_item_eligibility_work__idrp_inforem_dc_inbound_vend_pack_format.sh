...
#<>                                HEADER                                   <>#
###############################################################################
# SCRIPT NAME:         perform_item_eligibility_work__idrp_inforem_dc_inbound_vend_pack_format.sh
# AUTHOR NAME:         Abhijeet Shingate
# CREATION DATE:       Mon Oct 14 05:10:25 EDT 2013
...
module_relative_path=item_eligibility/work__idrp_inforem_dc_inbound_vend_pack

#this is the name of the script without extension
script_name=perform_item_eligibility_work__idrp_inforem_dc_inbound_vend_pack_format

#pig params file
pig_schema_file=${PIG_SCHEMAS_DIR}/item_eligibility/work__idrp_inforem_dc_inbound_vend_pack.schema

#pig params file
pig_params_file=${PIG_PARAMS_DIR}/${module_relative_path}/perform_item_eligibility_work__idrp_inforem_dc_inbound_vend_pack.param
...
work_formatted_dir=${WORK__IDRP_INFOREM_DC_INBOUND_VEND_PACK_FORMATTED_WORK_LOCATION}

#smith location
smith_dir=${WORK__IDRP_INFOREM_DC_INBOUND_VEND_PACK_LOCATION}
###############################################################################
#<>                                  BODY                                   <>#
###############################################################################
...
fn_delete_hadoop_directory_if_it_already_exist "${work_formatted_dir}"

#execute the pig script
fn_execute_pig -m ${pig_schema_file} -m ${pig_params_file} ${pig_script_file}

#remove target directory if it already exists
fn_delete_hadoop_directory_if_it_already_exist "${smith_dir}"

#copy work data to smith
fn_copy_hadoop_directory_if_target_location_does_not_exists "${work_formatted_dir}" "${smith_dir}"
