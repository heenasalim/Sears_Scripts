...
ls -t $MULTI_INSTANCE_1_DIR/IE_ITEM_LOC.IETimePhased*.dat > /dev/null 2>&1
if [ $? -ne 0 ]; then
echo "INSTANCE_1_FILE MISSING"
exit 1
fi
INSTANCE_1_FILE=`ls -t $MULTI_INSTANCE_1_DIR/IE_ITEM_LOC.IETimePhased*.dat | head -1`

ls -t $MULTI_INSTANCE_2_DIR/IE_ITEM_LOC.IETimePhased*.dat > /dev/null 2>&1
if [ $? -ne 0 ]; then
echo "INSTANCE_2_FILE MISSING"
exit 1
fi
INSTANCE_2_FILE=`ls -t $MULTI_INSTANCE_2_DIR/IE_ITEM_LOC.IETimePhased*.dat | head -1`

ls -t $MULTI_INSTANCE_3_DIR/IE_ITEM_LOC.IETimePhased*.dat > /dev/null 2>&1
if [ $? -ne 0 ]; then
echo "INSTANCE_3_FILE MISSING"
exit 1
fi
INSTANCE_3_FILE=`ls -t $MULTI_INSTANCE_3_DIR/IE_ITEM_LOC.IETimePhased*.dat | head -1`

ls -t $MULTI_INSTANCE_4_DIR/IE_ITEM_LOC.IETimePhased*.dat > /dev/null 2>&1
if [ $? -ne 0 ]; then
echo "INSTANCE_4_FILE MISSING"
exit 1
fi
INSTANCE_4_FILE=`ls -t $MULTI_INSTANCE_4_DIR/IE_ITEM_LOC.IETimePhased*.dat | head -1`

sed '1d ;$d' $INSTANCE_1_FILE | hadoop fs -put - $WORK__IDRP_ELIGIBLE_ITEM_LOCATION_CURRENT_INCOMING_LOCATION/instance_1
sed '1d ;$d' $INSTANCE_2_FILE | hadoop fs -put - $WORK__IDRP_ELIGIBLE_ITEM_LOCATION_CURRENT_INCOMING_LOCATION/instance_2
sed '1d ;$d' $INSTANCE_3_FILE | hadoop fs -put - $WORK__IDRP_ELIGIBLE_ITEM_LOCATION_CURRENT_INCOMING_LOCATION/instance_3
sed '1d ;$d' $INSTANCE_4_FILE | hadoop fs -put - $WORK__IDRP_ELIGIBLE_ITEM_LOCATION_CURRENT_INCOMING_LOCATION/instance_4
