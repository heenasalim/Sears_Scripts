...
# SCRIPT NAME:         perform_item_eligibility_smith__idrp_time_phased_status_current_previous.sh
...
fn_mkdir_hadoop_directory_if_it_does_not_exist $SMITH__IDRP_TIME_PHASED_STATUS_PREVIOUS_LOCATION

fn_delete_hadoop_directory_if_it_already_exist $SMITH__IDRP_TIME_PHASED_STATUS_PREVIOUS_TODAY_LOCATION

for old_file in `hadoop fs -ls $SMITH__IDRP_TIME_PHASED_STATUS_PREVIOUS_LOCATION | grep -v "Found" | tr -s " " "|" | cut -d"|" -f 8 | sort -r | tail -n +7`; do
fn_delete_hadoop_directory_if_it_already_exist $old_file
done 

fn_copy_hadoop_directory_if_target_location_does_not_exists $SMITH__IDRP_TIME_PHASED_STATUS_CURRENT_LOCATION $SMITH__IDRP_TIME_PHASED_STATUS_PREVIOUS_TODAY_LOCATION
