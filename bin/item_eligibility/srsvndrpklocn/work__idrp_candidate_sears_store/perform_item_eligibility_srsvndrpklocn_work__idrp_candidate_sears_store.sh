...
# SCRIPT NAME:         perform_item_eligibility_srsvndrpklocn_work__idrp_candidate_sears_store
# AUTHOR NAME:         Neera Singh
# CREATION DATE:       Tue July 08 09:37:59 EST 2014
...
module_relative_path=item_eligibility/srsvndrpklocn/work__idrp_candidate_sears_store

#this is the name of the script without extension
script_name=perform_item_eligibility_srsvndrpklocn_work__idrp_candidate_sears_store
...
. ${PIG_SCHEMAS_DIR}/gold__inventory_rim_daily_current.schema 
...
fn_non_zero_file_check $module_relative_path "$WORK__IDRP_DUMMY_VEND_WHSE_REF_LOCATION $GOLD__INVENTORY_RIM_DAILY_CURRENT_LOCATION/data_center* $WORK__IDRP_SEARS_LOCATION_XREF_LOCATION $SMITH__IDRP_KSN_ATTRIBUTE_CURRENT_LOCATION $GOLD__INVENTORY_SEARS_DC_ITEM_FACILITY_CURRENT_LOCATION $GOLD__ITEM_APRK_CURRENT_LOCATION" $CURRENT_DATE

. ${PIG_SCHEMAS_DIR}/item_eligibility/srsvndrpklocn/work__idrp_candidate_sears_store.schema

#delete the output HDFS directory
fn_delete_hadoop_directory_if_it_already_exist $WORK__IDRP_CANDIDATE_SEARS_STORE_LOCATION

#delete the output HDFS error directory
fn_delete_hadoop_directory_if_it_already_exist $WORK__IDRP_CANDIDATE_SEARS_STORE_MISSING_SOURCE_ERROR_LOCATION

#execute the pig script
fn_execute_pig -m ${PIG_SCHEMAS_DIR}/gold__inventory_rim_daily_current.schema -m ${PIG_SCHEMAS_DIR}/gold__item_aprk_current.schema -m ${PIG_SCHEMAS_DIR}/smith__idrp_ksn_attribute_current.schema -m ${PIG_SCHEMAS_DIR}/gold__inventory_sears_dc_item_facility_current.schema -m ${PIG_SCHEMAS_DIR}/item_eligibility/work__idrp_dummy_vend_whse_ref.schema -m ${PIG_SCHEMAS_DIR}/item_eligibility/srsvndrpklocn/work__idrp_sears_location_xref.schema -m ${PIG_SCHEMAS_DIR}/item_eligibility/srsvndrpklocn/work__idrp_candidate_sears_store.schema ${pig_script_file}
