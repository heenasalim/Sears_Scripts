...
# SCRIPT NAME:         perform_item_eligibility_smith__idrp_time_phased_conversions_ie_rim_status_update.sh
...
SCRIPT_NAME=perform_item_eligibility_smith__idrp_time_phased_conversions_ie_rim_status_update
...
ls -t $MULTI_INSTANCE_1_DIR/IE_RIM_STATUS_UPDATE.IETimePhased*.dat > /dev/null 2>&1
if [ $? -ne 0 ]; then
echo "INSTANCE_1_FILE MISSING"
exit 1
fi
INSTANCE_1_FILE=`ls -t $MULTI_INSTANCE_1_DIR/IE_RIM_STATUS_UPDATE.IETimePhased*.dat | head -1`

ls -t $MULTI_INSTANCE_2_DIR/IE_RIM_STATUS_UPDATE.IETimePhased*.dat > /dev/null 2>&1
if [ $? -ne 0 ]; then
echo "INSTANCE_2_FILE MISSING"
exit 1
fi
INSTANCE_2_FILE=`ls -t $MULTI_INSTANCE_2_DIR/IE_RIM_STATUS_UPDATE.IETimePhased*.dat | head -1`

ls -t $MULTI_INSTANCE_3_DIR/IE_RIM_STATUS_UPDATE.IETimePhased*.dat > /dev/null 2>&1
if [ $? -ne 0 ]; then
echo "INSTANCE_3_FILE MISSING"
exit 1
fi
INSTANCE_3_FILE=`ls -t $MULTI_INSTANCE_3_DIR/IE_RIM_STATUS_UPDATE.IETimePhased*.dat | head -1`

ls -t $MULTI_INSTANCE_4_DIR/IE_RIM_STATUS_UPDATE.IETimePhased*.dat > /dev/null 2>&1
if [ $? -ne 0 ]; then
echo "INSTANCE_4_FILE MISSING"
exit 1
fi
INSTANCE_4_FILE=`ls -t $MULTI_INSTANCE_4_DIR/IE_RIM_STATUS_UPDATE.IETimePhased*.dat | head -1`

fn_delete_linux_file_if_exists $STAGING_DIR/DRPP.HTRANSAC.IESTATUS

cat $INSTANCE_1_FILE  >>  $STAGING_DIR/DRPP.HTRANSAC.IESTATUS
cat $INSTANCE_2_FILE  >>  $STAGING_DIR/DRPP.HTRANSAC.IESTATUS
cat $INSTANCE_3_FILE  >>  $STAGING_DIR/DRPP.HTRANSAC.IESTATUS
cat $INSTANCE_4_FILE  >>  $STAGING_DIR/DRPP.HTRANSAC.IESTATUS
